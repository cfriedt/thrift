...
TCoapClient::TCoapClient(boost::shared_ptr<TTransport> transport,
                         std::string host,
                         std::string path)
  : TCoapTransport(transport), host_(host), path_(path) {
}
...
void TCoapClient::parseHeader(char* header) {
...
bool TCoapClient::parseStatusLine(char* status) {
  char* http = status;
